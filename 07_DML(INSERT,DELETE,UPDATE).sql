...
/*
    2) INSERT INTO 테이블명(컬럼명, 컬럼명, 컬러명) VALUES (값, 값, 값)
    -> 컬럼을 직접 제시하여 해당 컬럼에 대한 값만을 추가
        제시되지 않은 칼럼에 대한 값은 기본적으로 NULL 값이 저장
            기본값에 대한 옵션(DEFAULT) 이 있는 경우 해당 기본값으로 저장됨.
        ->NOT NULL 제약조건이 있는 컬럼의 경우 직접 칼럼을 제시해 값을 추가하거나,
                (DEFAULT 옵션을 설정해야 함! (그렇지않으면 오류발생!))
*/
...
-- 부서코드가 'D1'인 사원의 사번, 사원명, 부서코드, 사수사번, 입사일
...
/*
    UPDATE : 테이블에 저장되어있는 기존 데이터의 값을 변경하는 구분
    
    UPDATE XPDLQMFAUD
        SET 컬럼 = 값,
            컬럼 = 값,,,
    [WHERE 조건식] --> WHERE 절을 생략할 경우 테이블의 모든 행의 SET절에 제시한 컬럼의 데이터가 변경
...
WHERE EMP_NAME = '방명수';
SELECT * FROM EMP_TABLE WHERE EMP_NAME IN ('방명수','유지식');
...
FROM EMP_TABLE
    ...
    JOIN LOCATION ON LOCATION_ID = LOCAL_CODE
WHERE LOCAL_NAME LIKE 'ASIA%');
SELECT * FROM EMP_TABLE WHERE BONUS =0.3;
--------------------------
/*
    DELETE : 테이블에 기존에 저장된 데이터를 삭제할때 사용하는 구문
    DELETE FROM 테이블명
   [ WHERE 조건식]; --> WHERE 절 생략 시 모든 데이터를 삭제!
*/
...
-- 부서 테이블에서 D1인 부서 삭제
DELETE FROM DEPARTMENT WHERE DEPT_ID = 'D1';  -- 사용중일 때는 삭제가 안된다.
