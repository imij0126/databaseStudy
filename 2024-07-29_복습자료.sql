...
SELECT EMP_NAME,DEPT_NAME FROM EMPLOYEES JOIN DEPARTMENTS USING (DEPT_ID) WHERE DEPT_ID =2;

-- 모든 직원의 평균 급여를 계산 
SELECT ROUND(AVG(SALARY),1) FROM EMPLOYEES;

-- 부서별 직원 수를 계산하고, 직원 수가 1명 이상인 부서만 조회 
SELECT DEPT_ID,COUNT(DEPT_ID) FROM EMPLOYEES GROUP BY DEPT_ID HAVING COUNT(*) >= 1;
...
INSERT INTO EMPLOYEES VALUES (107,'김민지',6200,TO_DATE('2024-07-29','YYYY-MM-DD'),3);
...
ALTER TABLE EMPLOYEES ADD EMAIL VARCHAR2(100); -- DDL은 실행되는 순간 COMMIT 이 된다.
...
FROM EMPLOYEES JOIN DEPARTMENTS USING(DEPT_ID) GROUP BY DEPT_NAME HAVING AVG(SALARY) >= 6000;
...
-- 뷰 EMP_VIEW를 삭제
DROP VIEW EMP_VIEW; -- VIEW 삭제는 무조건 DROP VIEW!!!!
-- EMPLOYEES 테이블을 삭제
DROP TABLE EMPLOYEES;
ROLLBACK;