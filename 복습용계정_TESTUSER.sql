...
-- 나이가 30대인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM BIRTHDATE) + 1 BETWEEN 30 AND 39;
-- 광역시에 거주 중인 사람들의 정보 조회
SELECT SUBSTR(ADDRESS, 3,3) "주소" , NAME FROM CUSTOMER WHERE SUBSTR(ADDRESS, 3 ,3) = '광역시' ;
SELECT * FROM CUSTOMER WHERE ADDRESS LIKE '%광역시%';
-- 이름이 2자인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE LENGTH(NAME) = 2;
-- 이름, 생년월일, 나이 정보 조회
SELECT NAME, BIRTHDATE,EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM BIRTHDATE) + 1  "나이"  FROM CUSTOMER;