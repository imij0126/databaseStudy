/*
    ...
    SQL 문장 내 변수 정의, 조건, 반복 등을 지원해 SQL 단점 보완
    다수의 SQL문을 한번에 실행가능
...
DECLARE 
    ...
    PI CONSTANT NUMBER := 3.14; -- PI 라는 이름의 NUMBER 타입 상수선언 및 3.14 값으로 초기화
BEGIN
    -- 변수에 값을 대입
...
BEGIN
    ...
    INTO EID, ENAME, SAL               -- 각 칼럼의 결과 값을 변수에 대입
    FROM EMPLOYEE
    --WHERE EMP_ID = 200;
    WHERE EMP_ID = &사번;     -- 사번을 입력받아 조회한 후 결과를 변수에 대입
...
/*
    ...
    사용자가 입력한 사번의 사원 정보를 조회해 변수에 담아 출력
...
/*
    * ROW 타입 변수
        : 테이블의 한 행에 대한 모든 컬럼값을 한번에 담을수 있는 변수
...
/*
    ...
    ** 조건문 **
        - 단일 IF문 : IF 조건식 THEN 실행 내용 END IF;
        - IF/ELSE문 : IF 조건식 THEN 조건식이 참일때 실행내용 ELSE 조건식이 거짓일 때 실행내용 END IF;
        - IF/ELSIF 문 : IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ... [ELSE 실행내용] <- 생략가능 END IF;
...
BEGIN
    ...
    IF BONUS IS NULL THEN DBMS_OUTPUT.PUT_LINE('보너스를 받지 않는 사원입니다.' );
...
/*
    ...
    BEGIN
        ...
        -- 조회 후 해당 값을 변수에 저장
        -- 이때, NCODE 값이 KO이면 '국내팀'을 TEAM 변수에 저장
                           KO 가 아닐 경우 '해외팀'을 TEAM 변수에 저장
        -- 출력 형식 : 사번, 이름,부서, 소속 --> 한줄 또는 여러줄 출력
*/
SET SERVEROUTPUT ON;
...
BEGIN 
    ...
    IF GRADE = 'F'
        THEN DBMS_OUTPUT.PUT_LINE('재시험입니다.');
    END IF;
END;
/
DECLARE
...
-- FOR문 활요
BEGIN 
    FOR I IN 1..5 -- 반대로 하고 싶으면 FOR I IN REVERSE 1..5
    LOOP
...
/*
    * 예외처리부
    ...
    EXCEPTION
        ...
        * 오라클 미리 정의한 예외 -> 시스템 예외
            - NO_DATA_FOUND : 조회된 결과가 없을 때
            - TOO_MANY_ROWS : 조회된 결과가 여러 행일 때 (=> 변수에 대입..)
            - ZERO_DIVIDE : 0으로 값을 나누려고 할 때
            - DUP_VAL_ON_INDEX : UNIQUE 조건에 위배 될 때 (중복되는 경우)
            ...
        * OTHERS : 어떤 예외든 발생되었을 때
*/
-- 사용자에게 숫자를 입력받아 10으로 나눈 결과 출력 (0이 입력되었을때는 오류가 발생될 수 있다..)
DECLARE 
    NUM NUMBER ;
BEGIN
    NUM := &숫자;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('이름 : ' || ENAME);
EXCEPTION 
    WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('없는 값입니다.');
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('너무 많은 수의 행을 추출합니다.');
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('이상한거 입력하셨네요');
END;
/
