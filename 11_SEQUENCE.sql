/*
    시퀸스 (SEQUENCE) : 자동으로 번호를 발생시켜주는 역할을 하는 객체
...
-- 시작번호를 300,증가값은 5, 최대값은 310, 순환 X, 캐시메모리 X
...
/*
    * 시퀀스 사용하기 * 
    ...
    - 시퀀스명.NEXTVAL : 시퀀스 값에 일정 값을 증가해 발생한 결과값
                        현재 시퀀스 값에 INCREMENT BY 설정된 값 만큼 증가된 값
*/

SELECT SEQ_EMPNO.CURRVAL FROM DUAL; -- 오류 발생! NEXTVAL를 한번도 수행하지 않은 이상 CURRVAL는 사용할 수 없음!

SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 생성후 처음 수행 시 시작값 : 300
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; -- 300
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 305
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 310
-- SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- MAX 값이 310 이므로 실행 안됨
SELECT * FROM USER_SEQUENCES;
/*
    * 시퀀스 구조 변경하기 *
