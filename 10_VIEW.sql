/*
    * VIEW (뷰)
      : SELECT 문을 통해 얻어진 결과물을 저장해둘수 있는 객체
...
JOB_NAME 직급명,DECODE(SUBSTR(EMP_NO,8,1), 1, '남', 2, '여') 성별,
 EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) 근무년수
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE);

-- 위에 조회한 쿼리를 뷰에 저장 (VW_EMP_JOB)
...
----------------------------------------------------------
/*
    생성된 뷰를 통해 DML(INSERT/UPDATE/DELETE) 을 사용해보기
...
-- VW_JOB 뷰를 사용해 데이터를 추가 (DML-INSERT)
INSERT INTO VW_JOB VALUES ('J8','인턴');
-- VW_JOB 뷰를 사용해 데이터 변경 (DML - UPDATE)
UPDATE VW_JOB
SET JOB_NAME = '알바'
WHERE JOB_CODE = 'J8';
-- VW_JOB 뷰를 사용해 데이터 삭제(DML-DELETE)
DELETE FROM VW_JOB
WHERE JOB_CODE = 'J8';
/*
    * DML 명령어로 조작이 불가능한 경우가 많음!
    1) 뷰에 정의되지 않은 컬럼을 조작하려고 하는 경우
    2) 뷰에 정의되어 있지 않고 베이스테이블에서 NOT NULL 제약조건이 지점되어 있는 경우
    3) 산술연산식 또는 함수식으로 정의가 되어 있는 경우
    4) DISTINCT 구문이 포함되어 있는 경우
    5) JOIN을 이용해 여러 테이블을 연결시킨 경우
...
DELETE FROM VW_EMP WHERE EMP_ID = 200; -- READ ONLY 옵션에 의해 DML 사용불가!
